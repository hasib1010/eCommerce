...
const Cart = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed top-0 right-0 w-full md:w-1/3 bg-white text-black shadow-lg h-full z-40">
            <button onClick={onClose} className="absolute top-4 right-4 text-5xl font-bold">&times;</button>
            <div className='p-6 flex flex-col h-full'>
                <h2 className='text-3xl font-bold mb-6'>Shopping Cart</h2>
                <div
                    className="flex-1   z-[5000]"
                ...
                >
                    {state.items.length === 0 ? (
                        <div className='flex flex-col items-center justify-center h-full'>
                            ...
                            <Link onClick={onClose} to={'/AllProducts'} className='w-fit bg-gray-700 text-white text-center lg:text-5xl lg:p-5 p-2 rounded-3xl'>Shop Now</Link>
                        </div>
                    ) : (
                        <ul className='space-y-1   '>
                            {state.items.map(item => (
                                <li key={`${item.id}-${item.size}-${item.color}`} className='border p-4 rounded-lg flex items-center gap-4'>
                                    <img src={item.thumbnailImage} alt={item.name} className='lg:w-24 lg:h-24 h-16 object-cover rounded' />
                                    <div className='flex-1 '>

                                        <h3 className='text-sm font-semibold'>{item.name}</h3>
                                        <div className="flex items-center gap-5 text-sm">
                                            <p className='text-gray-600'>Size: {item.size}</p>
                                            <p className='text-gray-600'>Color: {item.color}</p> </div>
                                        <p className='text-sm font-bold'>${parseFloat(item.price).toFixed(2)}</p>
                                        <div className='flex items-center gap-2 mt-2'>
                                            <button onClick={() => handleDecreaseQuantity(item)} className='bg-gray-300 px-2 rounded'>-</button>
                                            <span className='text-lg font-bold'>{item.quantity}</span>
                                            <button onClick={() => handleIncreaseQuantity(item)} className='bg-gray-300 px-2 rounded'>+</button>
                                            <button onClick={() => handleRemoveItem(item)} className='bg-red-500 text-white px-2 py-1 rounded'>Remove</button>
                                        </div>
                                    </div>
                                </li>
                            ))}
